// code to generate a MC sample for gamma boson
...
const double shat_min = pow( 2*pt_min , 2.);
double shat_max = s;
...
double xsect_qq(double shat, double that, double uhat)
{
  return (that*that+uhat*uhat)/(that*uhat); 
}

// define the partonic cross section (quark-gluon)
double xsect_qg(double shat, double that, double uhat)
{
  return (shat*shat+uhat*uhat)/(shat*uhat);
}
...
void gammaMC(){
  ...
  TFile* hfile = new TFile( "MC_res_gamma.root", "RECREATE", "gammaMC" );

  //defining the parameters to be used here
  double x1, x2, y, y_gg, shat, cosalpha, pLhat, pt;
  ...
  TTree* gamma = new TTree( "gammaMC", Form("gamma (sqrts = %.1f TeV)", sqrts/1000.) );
    
  gamma->Branch( "x1",       &x1,       "x1/D" );
  gamma->Branch( "x2",       &x2,       "x2/D" );
  gamma->Branch( "shat",     &shat,     "shat/D" );
  gamma->Branch( "cosalpha", &cosalpha, "cosalpha/D" );
  gamma->Branch( "pLhat",    &pLhat,    "pLhat/D" );
  gamma->Branch( "pt",       &pt,       "pt/D" );
  gamma->Branch( "y",        &y,        "y/D"  );
  gamma->Branch( "y_gg",     &y_gg,     "y_gg/D"  );

  /*    gamma->Branch("costh_CS",   &costh_CS,   "costh_CS/D");
	gamma->Branch("phi_CS",     &phi_CS,     "phi_CS/D"  );
	gamma->Branch("phith_CS",   &phith_CS,   "phith_CS/D");
	gamma->Branch("costh_HX",   &costh_HX,   "costh_HX/D");
	gamma->Branch("phi_HX",     &phi_HX,     "phi_HX/D"  );
	gamma->Branch("phith_HX",   &phith_HX,   "phith_HX/D");
	gamma->Branch("costh_PX",   &costh_PX,   "costh_PX/D");
	gamma->Branch("phi_PX",     &phi_PX,     "phi_PX/D"  );
	gamma->Branch("phith_PX",   &phith_PX,   "phith_PX/D");
	gamma->Branch("costh_ggHX", &costh_ggHX, "costh_ggHX/D");
	gamma->Branch("phi_ggHX",   &phi_ggHX,   "phi_ggHX/D"  );
	gamma->Branch("phith_ggHX", &phith_ggHX, "phith_ggHX/D");

	gamma->Branch("cosalpha_inv", &cosalpha_inv, "cosalpha_inv/D");
	gamma->Branch("axAngle", &axAngle, "axAngle/D");*/
  
  // weights to be applied to the events when any distribution is plotted
  gamma->Branch( "jac",       &jac,       "jac/D"  );
  gamma->Branch( "w_uub",     &w_uub,     "w_uub/D"  );
  gamma->Branch( "w_ddb",     &w_ddb,     "w_ddb/D"  );
  gamma->Branch( "w_ug",      &w_ug,      "w_ug/D"  );
  gamma->Branch( "w_dg",      &w_dg,      "w_dg/D"  );
  gamma->Branch( "w_ubg",     &w_ubg,     "w_ubg/D"  );
  gamma->Branch( "w_dbg",     &w_dbg,     "w_dbg/D"  );
  /*gamma->Branch( "w_CS_tr",   &w_CS_tr,   "w_CS_tr/D"  );
  gamma->Branch( "w_HX_tr",   &w_HX_tr,   "w_HX_tr/D"  );
  gamma->Branch( "w_PX_tr",   &w_PX_tr,   "w_PX_tr/D"  );
  gamma->Branch( "w_ggHX_tr", &w_ggHX_tr, "w_ggHX_tr/D"  );
  gamma->Branch( "w_CS_lg",   &w_CS_lg,   "w_CS_lg/D"  );
  gamma->Branch( "w_HX_lg",   &w_HX_lg,   "w_HX_lg/D"  );
  gamma->Branch( "w_PX_lg",   &w_PX_lg,   "w_PX_lg/D"  );
  gamma->Branch( "w_ggHX_lg", &w_ggHX_lg, "w_ggHX_lg/D"  );*/
  
  gamma->Branch( "pick",     &pick,     "pick/D"  );
  ...
  for( int i_event = 1; i_event <= n_events; i_event++ ){
    ...
    double x_min = shat / s; // min of the parton fractional momentum; this expression derives from x1*x2 = shat / s and |x| < 1
    ...
    // p, pT, pL, E -> all partonic
    double phat = sqrt(shat)/2.; 
    pLhat = phat*cosalpha;
    double Ehat = sqrt(shat)/2.;
    pt = phat*sqrt(1-cosalpha*cosalpha);

    // y expressions + pL (pT^hat = pT)
    double y_hat = 0.5*log((Ehat + pLhat)/(Ehat - pLhat));
    y_gg = 0.5*log(x1/x2);
    y = y_hat + y_gg;
    double pL=pt*sinh(y);
    ...
    // from the sigma expression we get an shat^2
    double sigma_aux = 1./(shat*shat);
    
    // dx1 dx2 dt^hat = Jac * dlogx dshat dcosalpha^hat
    // Jac = shat / (2s)
    jac =  shat/(2.*s) ;
    // the PDFs for the partons are symmetrized in x1, x2
    double uubx = 0.5*(func_upquarkPDF(x1, shat)*func_upaquarkPDF(x2, shat)+func_upaquarkPDF(x1, shat)*func_upquarkPDF(x2, shat));
    double ddbx = 0.5*(func_downquarkPDF(x1, shat)*func_downaquarkPDF(x2, shat)+func_downaquarkPDF(x1, shat)*func_downquarkPDF(x2, shat));
    double ugx = 0.5*(func_upquarkPDF(x1, shat)*func_gluonPDF(x2, shat)+func_gluonPDF(x1, shat)*func_upquarkPDF(x2, shat));
    double dgx = 0.5*(func_downquarkPDF(x1, shat)*func_gluonPDF(x2, shat)+func_gluonPDF(x1, shat)*func_downquarkPDF(x2, shat));
    double ubgx = 0.5*(func_upaquarkPDF(x1, shat)*func_gluonPDF(x2, shat)+func_gluonPDF(x1, shat)*func_upaquarkPDF(x2, shat));
    double dbgx = 0.5*(func_downaquarkPDF(x1, shat)*func_gluonPDF(x2, shat)+func_gluonPDF(x1, shat)*func_downaquarkPDF(x2, shat));
      
    // the full weight functions are the product of all this
    w_uub = sigma_aux * uubx * 8./9.    * xsect_qq(shat, that, uhat);
    w_ddb = sigma_aux * ddbx * 8./9.    * xsect_qq(shat, that, uhat);
    w_ug  = sigma_aux * ugx  * (-1./3.) * xsect_qg(shat, that, uhat);
    w_dg  = sigma_aux * dgx  * (-1./3.) * xsect_qg(shat, that, uhat);
    w_ubg = sigma_aux * ubgx * (-1./3.) * xsect_qg(shat, that, uhat);
    w_dbg = sigma_aux * dbgx * (-1./3.) * xsect_qg(shat, that, uhat);
	...
	*lepP = lepton_DILEP_xyz;
    ...
    accepted = accepted && (pt > pt_min) && (pt < pt_max) ;
