...
// generates in the lab variables pT, y, y_X
...
void ZMC_pT(){
  ...
  TFile* hfile = new TFile( "MC_res_Z_pT.root", "RECREATE", "ZMC" );

  //defining the parameters to be used here
  double x1[n_s_val], x2[n_s_val], y_gg[n_s_val];
  double sstar, cosalpha, pLstar, xi, y, y4;
  ...
  for(int i = 0; i < n_s_val; i++) {
    ...
    zbar[i]->Branch( "y",        &y,        "y/D"  );
    zbar[i]->Branch( "y4",       &y4,        "y4/D"  );
  ...
  for( int i_event = 1; i_event <= n_events; i_event++ ){
    ...
    for(int i = 0; i < n_s_val; i++) {
      // from the change of vars from dx1 dx2 dt to dpT dy dy4, we get a Jacobian
      double fJacobian =  ((2.*pT*s_hat) / (s_hat-M*M)) * s_hat/s[i] ;
	...
	*lepP = lepton_DILEP_xyz;
    ...
    for(int i = 0; i< n_s_val; i++) {
      ...
      //if(((sstar > sstar_min) && (sstar < sstar_max))  == false) cout << "sstar condition not met" << endl; (x1[i] > s_hat/s[i])
      accepted = accepted && (x1[i] > s_hat/s[i]) && (x2[i] > s_hat/s[i]) && (x1[i] < 1.) && (x2[i] < 1.);
      //if(((x1[i] > s_hat/s[i]) && (x2[i] > s_hat/s[i]) && (x1[i] < 1.) && (x2[i] < 1.)) == false) cout << "x1, x2 condition not met" << endl;
      accepted = accepted && (cosalpha < 1 ) && (cosalpha > -1);
      //if(((cosalpha < 1 ) && (cosalpha > -1)) == false) cout << "cosalpha condition not met" << endl;
