...
// generates in the lab variables pT, y, cosalpha
...
void gammaMC_pT(){
  ...
  TFile* hfile = new TFile( "MC_res_gamma_pT.root", "RECREATE", "gammaMC" );
  ...
  double jac, w_uub, w_ddb, w_ug, w_dg, w_ubg, w_dbg;
  ...
  for( int i_event = 1; i_event <= n_events; i_event++ ){
    ...
    double phat = pt / sqrt(1.-cosalpha*cosalpha);
    double sqhat = 2.*phat;
    shat = sqhat*sqhat;
    ...
    // Jac = x1 x2 / pT *  s^hat (massless limit)
    jac =  (x1*x2 / pt) * shat;
	...
	*/
    ...
    bool accepted = true;
    accepted = accepted && (shat > shat_min) && (shat < shat_max) ;
    accepted = accepted && (x1 > shat/s) && (x2 > shat/s) && (x1 < 1.) && (x2 < 1.);
