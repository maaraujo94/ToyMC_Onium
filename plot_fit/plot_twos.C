...
void plot_twos()
{
  ...
  for(int i_s = 0; i_s < n_states; i_s++) {
	...
	for(int j_y = 0; j_y < xi_n; j_y++) {
	  ...
	  // save plot
	  leg->Draw();
	  can->SaveAs(Form("%splots_%s/xi_%s_y%d.pdf", type.c_str(), dataName.c_str(), sqsName.c_str(), j_y+1));
	  can->Clear();
	}
	...
	can->SaveAs(Form("%splots_%s/y_dist_log_%s.pdf", type.c_str(), dataName.c_str(), sqsName.c_str()));
