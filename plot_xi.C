...
void plot_xi()
{
  ...
  // part 0 : variables
  const int n_y = 5;
  string name = "MC_res.root";
  double dataN = 8.;
  double histoN = 1.;
  // states available: "jpsi", "ups1", "psi2"
  string dataName = "psi2";
  
  double y_min[n_y] = {2, 2.5, 3, 3.5, 4};
  double y_max[n_y] = {2.5, 3, 3.5, 4, 4.5};
  
  double xi_min[n_y] = {0, 0, 0, 0, 0};
  double xi_max[n_y] = {6, 6, 6, 6, 6};

  double yax_min[n_y] = {1e-3, 1e-3, 1e-3, 1e-3, 1e-3};
  double yax_max[n_y] = {1e2, 1e2, 1e2, 1e2, 1e2};
  ...
  if (dataName == "jpsi") {
    ...
    string filelist[n_y] = {"data/LHCb_psi2_7_cs_y1.txt",
  ...
  for(int j_y = 0; j_y < n_y; j_y++) {    
    ...
    can->SaveAs(Form("plots/xi_new_%s.pdf", Form("LHCb_y%d", j_y)));
    can->Clear();
  }
}
